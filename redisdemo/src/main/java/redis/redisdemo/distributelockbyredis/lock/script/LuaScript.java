package redis.redisdemo.distributelockbyredis.lock.script;
...
public class LuaScript {
    ...
    public static void initLockScript() throws IOException {
        if (StringUtils.isEmpty(LOCK_SCRIPT)) {
            InputStream resourceAsStream = LuaScript.class.getResourceAsStream("/lock.lua");
            log.info("resourceAsStream:   {}",resourceAsStream);
            InputStream inputStream = Objects.requireNonNull(
                LuaScript.class.getResourceAsStream("/lock.lua"));
    ...
    public static void initUnLockScript() throws IOException {
        if (StringUtils.isEmpty(UN_LOCK_SCRIPT)) {
            InputStream inputStream = Objects.requireNonNull(
                LuaScript.class.getResourceAsStream("/unlock.lua"));
